...
public partial class InlineTranslation : Node //内嵌式翻译
{
	...
	{ 
		...
		bool success = WriteTranslationToFile(task.FilePath, task.ElementIndex, translatedText);

        if (success)
        {
            GD.Print($"翻译成功: {Path.GetFileName(task.FilePath)} [{task.ElementIndex}]");
        }
        else
        {
	...
	private bool WriteTranslationToFile(string filePath, int elementIndex, string translatedText)
	{
		try
		{
            ...
            string newJson = Json.Stringify(jsonDict); // 将更新后的字典转换回JSON字符串
